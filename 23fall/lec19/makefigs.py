...
axs[1].set_title('$H_{LP}(\omega)$')
plotspec(axs[2],omega,np.abs(X)*LI, xticks, xticklabels)
axs[2].set_title('$|Y(\omega)|=H_{LP}(\omega)|X(\omega)|$')
fig.tight_layout()
...
for c in range(3):
    ...
    axs[c,0].set_title('$H_{LP}(\omega)$, cutoff=%s'%(xticklabels[5+c]))
    ...
    axs[c,1].set_title('$h_{LP}[n]$, cutoff=%s'%(xticklabels[5+c]))
...
#############################################################################################
# Convolution of a noisy input signal with an ideal LPF.
fig = matplotlib.figure.Figure((5,4))
...
axs[2].clear()
h = l[(N-1+n):(n-1):-1]
axs[2].stem(h*brownian)
#axs[2].plot([0,N-1],[0,0],'k-',[0,1e-6],[-0.5,0.5],'k-')
axs[2].set_title('$h_{LP}[%d-m]x[m]$'%(n))
axs[3].clear()
axs[3].plot(np.arange(0,n+1),y[:(n+1)],'b-')
axs[3].plot([0,N-1],[0,0],'k-',[0,1e-6],[np.amin(y)-0.1,np.amax(y)+0.1],'k-')
axs[3].set_title('$y[n]=h_{LP}[n]*x[n]$')
axs[3].set_xlabel('$m$')
fig.tight_layout()
...
for c in range(3):
    ...
    axs[c,0].set_title('H_{HP}(\omega), cutoff=%s'%(xticklabels[5+c]))
    ...
    axs[c,1].set_title('$h_{HP}[n]$, cutoff=%s'%(xticklabels[5+c]))
    axs[c,1].set_xlim(-10,10)
...
#############################################################################################
# Video showing convolution of a noisy input signal with an ideal HPF.
fig = matplotlib.figure.Figure((5,4))
axs = fig.subplots(4,1,sharex=True)
cutoff = np.pi/2
h[nset==0]= 1 - cutoff/np.pi
...
for n in [int(N/2)]:
    ...
    axs[1].plot([0,(N-1)],[0,0],'k-',[0,1e-6],[-0.5,0.5],'k-')
    axs[1].set_title('$h_{HP}[%d-m]$'%(n))
    axs[2].clear()
    axs[2].stem(h[((N-1)+n):(n-1):-1]*brownian)
    #axs[2].plot([0,(N-1)],[0,0],'k-',[0,1e-6],[-0.5,0.5],'k-')
    axs[2].set_title('$h_{HP}[%d-m]x[m]$'%(n))
    axs[3].clear()
    axs[3].plot(np.arange(0,n+1),y[:(n+1)],'b-')
    axs[3].plot([0,(N-1)],[0,0],'k-',[0,1e-6],[np.amin(y)-0.1,np.amax(y)+0.1],'k-')
    axs[3].set_title('$y[n]=h_{HP}[n]*x[n]$')
    axs[3].set_xlabel('$m$')
    fig.tight_layout()
...
for c in range(2,-1,-1):   # Set it this way so b remains set for the video
    ...
    axs[c,0].set_title('$H_{BP}(\omega)$, cutoffs=%s, %s'%(cutoffs[c],cutoffs[c+1]))
    ...
    axs[c,1].set_title('$h_{BP}[n]$, cutoffs=%s, %s'%(cutoffs[c],cutoffs[c+1]))
    axs[c,1].set_xlim(-10,10)
...
for n in [int(N/2)]:
    ...
    axs[2].clear()
    axs[2].stem(b[((N-1)+n):(n-1):-1]*brownian)
    #axs[2].plot([0,(N-1)],[0,0],'k-',[0,1e-6],[-0.5,0.5],'k-')
    axs[2].set_title('$b_I[%d-m]x[m]$'%(n))
    axs[3].clear()
    axs[3].plot(np.arange(0,n+1),y[:(n+1)],'b-')
    axs[3].plot([0,(N-1)],[0,0],'k-',[0,1e-6],[np.amin(y)-0.1,np.amax(y)+0.1],'k-')
    axs[3].set_title('$y[n]=b_I[n]*x[n]$')
    axs[3].set_xlabel('$m$')
    fig.tight_layout()
...
axs[0,1].set_title('$H_{LP,i}(\omega)$, cutoff=%s'%(xticklabels[5]))
axs[0,0].plot(nset,li,'b-',nset,np.zeros(len(nset)),'k-',[0,1e-6],[-0.5,1],'k-')
axs[0,0].set_title('$h_{LP,i}[n]$, cutoff=%s'%(xticklabels[5]))
...
axs[1,1].set_title('$H_{LP}(\omega)$, cutoff=%s'%(xticklabels[5]))
axs[1,0].plot(narr,l,'b-',nset,np.zeros(len(nset)),'k-',[0,1e-6],[-0.5,1],'k-')
axs[1,0].set_title('Truncated $h_{LP}[n]$, cutoff=%s'%(xticklabels[5]))
...
axs[0].set_title('Truncated $h_{LP}[n]$, cutoff=%s'%(xticklabels[5]))
...
axs[2].set_title('Windowed Filter $w[n]h_{LP}[n]$, Length=%d'%(2*M+1))
...
LI[np.abs(omega)<=cutoff]=1
plotspec(axs[0,1],omega,LI,xticks,xticklabels)
axs[0,1].set_title('$H_{LP}(\omega)$, cutoff=%s'%(xticklabels[5]))
axs[0,0].plot(nset,li,'b-',nset,np.zeros(len(nset)),'k-',[0,1e-6],[-0.5,1],'k-')
axs[0,0].set_title('$h_{LP}[n]$, cutoff=%s'%(xticklabels[5]))
axs[0,0].set_xlim(-2*M,2*M)
axs[0,0].set_ylim(np.amin(li)-0.1,np.amax(li)+0.1)
LW = np.fft.fftshift(np.real(np.fft.fft(np.fft.fftshift(l*w))))
plotspec(axs[1,1],omeg,LW,xticks,xticklabels)
axs[1,1].set_title('Windowed $H_{LP}(\omega)$, cutoff=%s'%(xticklabels[5]))
axs[1,0].plot(narr,l*w,'b-',nset,np.zeros(len(nset)),'k-',[0,1e-6],[-0.5,1],'k-')
axs[1,0].set_title('Windowed $h_{LP}[n]$, cutoff=%s'%(xticklabels[5]))
axs[1,0].set_xlim(-2*M,2*M)
...
LI[np.abs(omega)<=cutoff] = 1
plotspec(axs[1],omega,LI,xticks,xticklabels)
axs[1].set_title('Magnitude of delayed filter = $|H_{LP}(\omega)|$')
phi = np.zeros(len(omega))
phi[np.abs(omega)<=cutoff]=-(M-0.5)*omega[np.abs(omega)<=cutoff]
plotspec(axs[2],omega,phi,xticks,xticklabels)
axs[2].set_title('Phase of delayed filter ∠$H_{LP}(\omega)$= -j%2.1f$\omega$'%(M-0.5))
fig.tight_layout()
...
axs[2].set_title('Windowed Delayed Filter $w[n]h_{LP}[n-%2.1f]$, Length=%d'%(M-0.5,2*M+1))
...
axs = fig.subplots(4,1,sharex=True)
cutoff = np.pi/3
l = np.sin(cutoff*(nset-0.5))/(np.pi*(nset-0.5))
w = np.zeros(len(nset))
w[np.abs(nset-0.5)<M] = 0.54-0.46*np.cos(2*np.pi*nset[np.abs(nset-0.5)<M]/(2*M-1))
l = l*w
y = np.convolve(brownian, l, mode='valid')
axs[0].plot(brownian)
axs[0].plot([0,N-1],[0,0],'k-',[0,1e-6],[np.amin(brownian)-0.1,np.amax(brownian)+0.1],'k-')
...
axs[1].clear()
axs[1].plot(l[(N-1+n):(n-1):-1])
axs[1].plot([0,N-1],[0,0],'k-',[0,1e-6],[-np.amax(np.abs(l)),np.amax(np.abs(l))],'k-')
axs[1].set_title('$h_{LP}[%d-m]$'%(n))
axs[2].clear()
h = l[(N-1+n):(n-1):-1]
axs[2].stem(h*brownian)
#axs[2].plot([0,N-1],[0,0],'k-',[0,1e-6],[-0.5,0.5],'k-')
