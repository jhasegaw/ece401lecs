...
###########################################################################
# Aliased spectra: cosine
fig, ax = plt.subplots(1,1,figsize=(14,4))
ax.plot([-3.5,3.5],[0,0],'k--',[0,1e-6],[-0.75,0.75],'k--')
ax.stem([-0.75,0.75],[0.5,0.5])
ax.set_xticks([-0.75,0.75])
ax.set_xticklabels(['$-0.75\pi$','$0.75\pi$'],fontsize=18)
fig.savefig('exp/ct_cosine.png')

for i in range(46):
    fig, ax = plt.subplots(1,1,figsize=(14,4))
    ax.plot([-3.5,3.5],[0,0],'k--',[0,1e-6],[-0.75,0.75],'k--')
    f = 0.75+0.01*i
    freqs = [-4+f,-2-f,-2+f,-f,f,2-f,2+f,4-f]
    ax.stem(freqs,0.5*np.ones(len(freqs)))
    ax.set_xticks(freqs)
    ax.set_xticklabels(['$%2.2f\pi$'%(f) for f in freqs],fontsize=18)
    fig.savefig('exp/dt_cosine%d.png'%(i))

###########################################################################
# Aliased spectra: sine
fig, ax = plt.subplots(1,1,figsize=(14,4))
ax.plot([-3.5,3.5],[0,0],'k--',[0,1e-6],[-0.75,0.75],'k--')
ax.stem([-0.75,0.75],[-0.5,0.5])
ax.set_xticks([-0.75,0.75])
...
for i in range(46):
    ...
    ax.set_xticks(freqs)
    ax.set_xticklabels(['$%2.2f\pi$'%(f) for f in freqs],fontsize=18)
    fig.savefig('exp/dt_sine%d.png'%(i))
