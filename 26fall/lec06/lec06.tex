...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section[DAC]{Digital-to-Analog Conversion}
\setcounter{subsection}{1}
...
\begin{frame}
  \frametitle{DAC Aliasing}

  \begin{itemize}
    \item The discontinuities occur $F_s=\frac{1}{T_s}$ times seconds,
      with amplitudes modulated by the cosine at frequency $f$.  The
      result has noise components at $k F_s\pm f$ for every integer
      $k$.
    \item 
      For example, suppose we start with
      $x[n]=\cos\left(\frac{n\pi}{8}\right)$, and interpolate using a
      rectangular pulse at $8000$ samples/second, giving $f=\omega
      F_s/2\pi=1000$.  We wind up with
      \begin{displaymath}
        y(t) = a\cos\left(2\pi 1000 t\right)+b\cos\left(2\pi 7000t\right)+c\cos\left(2\pi 9000t\right)+\cdots,
      \end{displaymath}
...
\begin{frame}
  ...
  discontinuous, so it still has energy at the frequences $kF_s\pm f$.
  Images look pretty good like this, but audio signals still sound
  noisy (the ear is more sensitive to high frequencies than the eye).
\end{frame}
...
\begin{frame}
  ...
  \ldots which has \textbf{less} energy at the aliasing frequencies
  $kF_s\pm f$, but not \textbf{zero} energy.
\end{frame}
...
\begin{frame}
  ...
  \begin{displaymath}
    y(t) = \cos\left(2\pi 1000 t\right)
  \end{displaymath}
  Perfect!
\end{frame}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section[Upsampling]{Upsampling a digital signal}
\setcounter{subsection}{1}
