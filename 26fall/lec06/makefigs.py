...
h_pwc = np.zeros(2*zero_sample+1)
h_pwl = np.zeros(2*zero_sample+1)
for n in range(0,4):
    h_pwc[zero_sample+n]=1
    h_pwl[zero_sample+n]=0.25*(4-n)
    h_pwl[zero_sample-n]=0.25*(4-n)
h_sinc = np.zeros(2*zero_sample+1)
h_sinc[n_axis!=0] = np.sin(0.25*np.pi*n_axis[n_axis!=0]) / (np.pi*n_axis[n_axis!=0])
h_sinc[zero_sample] = 0.25
fig, ax = plt.subplots(3,1,figsize=(14,9))
ax[0].stem(n_axis,h_pwc)
ax[0].set_title('PWC Interpolation is like convolving with the PWC interpolator')
ax[1].stem(n_axis,h_pwl)
ax[1].set_title('PWL Interpolation is like convolving with the PWL interpolator')
ax[2].stem(n_axis,h_sinc)
ax[2].set_title('Sinc Interpolation makes an ideally smooth signal')
fig.tight_layout()
fig.savefig('exp/dt_interpolators.png')
