...
\title{Lecture 2: Linearity and Shift-Invariance}
...
\begin{frame}
  ...
  linear, but it's actually {\bf nonlinear}.  A system that would be
  linear except for the added constant is called an {\bf affine}
  system.
  \begin{align*}
...
\begin{frame}
  ...
  A system ${\mathcal H}$ is {\bf time-invariant} if these two
...
\begin{frame}
  ...
  {\bf Shift-invariance} generalizes {\bf time-invariance} to cases
  where the index variable, $n$, might mean something other than time
  (for example, it might mean spatial location, or row of a matrix, or
  whatever).  A system ${\mathcal H}$ is said to be {\bf
    shift-invariant} if and only if, for every $x_1[n]$,
  \begin{align*}
    x_1[n] &\stackrel{\mathcal H}{\longrightarrow} y_1[n]
  \end{align*}
  implies that
