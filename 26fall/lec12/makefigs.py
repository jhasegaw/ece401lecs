...
y = np.real(np.fft.ifft(np.fft.fft(h)*np.fft.fft(x)))
axs[2,0].stem(nset, y)
axs[2,0].set_title('$y[n]=x[n]*h[n]$ linear')
axs[2,0].set_xlabel('$n$')
...
ybuild = np.zeros(len(nset))
for n in range(N):
    hbuild = np.zeros(len(h))
    for m in mset:
        if n-m >= 0:
            hbuild[m] = h[n-m]
    axs[1,1].clear()
    axs[1,1].stem(nset,hbuild)
    axs[1,1].set_title('$h[%d-m]$'%(n))
    ybuild[n] = np.sum(hbuild * x)
    axs[2,1].clear()
    axs[2,1].stem(mset, ybuild)
    axs[2,1].set_title('$y[m]=x[m]*h[m]$ linear')
    axs[2,1].set_xlabel('$m$')
    axs[2,1].set_ylim([0,1.1*np.amax(y)])
    fig.tight_layout()
    fig.savefig('exp/linear%d.png'%(n))
